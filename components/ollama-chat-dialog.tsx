    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <DialogHeader className="shrink-0 border-b border-border px-4 py-3 pr-12 text-left">
          ...
          <DialogDescription>
            服务端转发至 OLLAMA_PROXY_URL；联网时调用 <code className="text-xs">/api/chat-with-tools</code>。
            若代理返回 HTML 或非标准 JSON，请检查隧道与路径。
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-3">
            {messages.length === 0 && !pending && (
              <p className="text-sm text-muted-foreground">
                关闭联网为普通对话；开启后走工具接口（需代理支持 JSON 或纯文本回复）。
              </p>
            )}
            {messages.map((m) => (
              <div
            ...
            {error && (
              <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive whitespace-pre-wrap break-words max-h-48 overflow-y-auto">
                {error}
              </div>
            )}
          </div>
